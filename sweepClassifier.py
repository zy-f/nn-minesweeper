...
class SweepClassifier(object):
    ...
    def train(self, batch, lr=1e-3, mini_bsz=32, batch_iters=1, printing=False, load_best=None):
        self.model.train()
        if load_best is not None:
            self.load_model(load_best)
        self.optim = torch.optim.Adam(self.model.parameters(), lr=lr, betas=(.8,.92))
